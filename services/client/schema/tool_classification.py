...
class ToolClassification(BaseModel):
    web_tools: List[StructuredTool] = Field(default_factory=list, description="""
...
web search tools, web crawling and scraping tools, etc. Only include the tools in this list which could not be fitted into any of the other specialized categories.
        """)
    weather_tools: List[StructuredTool] = Field(default_factory=list, description="""
...
weather forecast, wind speeds, precipitation, and historical weather data.
        """)
    map_tools: List[StructuredTool] = Field(default_factory=list, description="""
...
geographical area details like amenities, nearby amenities and public infrastructure details.
    ...
    hotel_stays_tools: List[StructuredTool] = Field(default_factory=list, description="""
